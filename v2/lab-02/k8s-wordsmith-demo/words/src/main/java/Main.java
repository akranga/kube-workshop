...
public class Main {
    public static void main(String[] args) throws Exception {
        ...
        server.createContext("/noun", handler(Suppliers.memoizeWithExpiration(() -> randomWord("nouns"),3,TimeUnit.SECONDS)));
        server.createContext("/verb", handler(Suppliers.memoizeWithExpiration(() -> randomWord("verbs"),3,TimeUnit.SECONDS)));
        server.createContext("/adjective", handler(Suppliers.memoizeWithExpiration(() -> randomWord("adjectives"),3,TimeUnit.SECONDS)));
        server.start();
    }
